...
export default function WheelDetailPage() {
  ...
  useEffect(() => {
    const fetchWheel = async () => {
      try {
        setLoading(true);
        const data = await ApiProvider.wheels.getDetail(id);
        setWheel(data.data || data);
      } catch (err) {
        console.error("Fetch error:", err);
        setError(err.message || "Failed to load wheel details");
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-semibold">Loading wheel details...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-600 font-semibold mb-6">{error || "Wheel not found"}</p>
          <button
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {wheel.description && (
              <div className="bg-white rounded-2xl p-6 border border-gray-200">
                ...
                <p className="text-gray-700 whitespace-pre-line leading-relaxed">{wheel.description}</p>
              </div>
            )}
          </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <div className="flex items-baseline gap-2">
                    <span className="text-3xl font-bold text-green-600">{formatPrice(wheel.price)}</span>
                    {wheel.customerPrice && (
                      <span className="text-lg text-gray-400 line-through">{formatPrice(wheel.customerPrice)}</span>
                    )}
                  </div>
                </div>

                {wheel.price && wheel.customerPrice && parseFloat(wheel.customerPrice) > parseFloat(wheel.price) && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                    <p className="text-sm font-semibold text-red-700">
                      Save ₹{(parseFloat(wheel.customerPrice) - parseFloat(wheel.price)).toLocaleString("en-IN")}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                {wheel.size && (
                  <div className="flex justify-between py-2 border-b border-gray-100">
                    ...
                    <span className="font-semibold text-gray-900">{wheel.size}</span>
                  </div>
                )}
                {wheel.brand && (
                  <div className="flex justify-between py-2 border-b border-gray-100">
                    <span className="text-gray-600">Brand</span>
                    <span className="font-semibold text-gray-900">{wheel.brand}</span>
                  </div>
                )}
                {wheel.quantity && (
                  <div className="flex justify-between py-2 border-b border-gray-100">
                    <span className="text-gray-600">Quantity</span>
                    <span className="font-semibold text-gray-900">{wheel.quantity}</span>
                  </div>
                )}
                {wheel.condition && (
                  <div className="flex justify-between py-2">
                    <span className="text-gray-600">Condition</span>
                    <span className="font-semibold text-gray-900 capitalize">{wheel.condition}</span>
                  </div>
                )}
              </div>
            </div>
