...
function BikeCard({ bike, onCardClick }) {
  ...
  const images = Array.isArray(bike.medias) ? bike.medias : [];
  const currentImage = images[imageIndex]?.media;
  const title = bike.name || bike.title || [bike.brandName, bike.model].filter(Boolean).join(" ") || "Bike Listing";
  const km = Number.parseInt(bike.km, 10);
  const kmText = Number.isFinite(km) ? `${km.toLocaleString("en-IN")} km` : "N/A km";
  const fuelText = bike.fuelType || "Petrol";
  const locationText = [bike.user?.city, bike.user?.state].filter(Boolean).join(", ") || bike.user?.shopName || "Location unavailable";
  const conditionText = bike.condition || "Good";
    ...
    >
      ...
      <div className="p-3.5">
        ...
        <div className="flex items-end justify-between mt-2.5">
          <div className="text-2xl leading-none font-black text-gray-800">{formatPriceCompact(bike.price)}</div>
          <div className="text-gray-600 text-xs">
            EMI at <span className="font-bold text-gray-800">{formatEmi(bike.price)}</span>
          </div>
        </div>
        ...
        {bike.customerPrice && parseFloat(bike.customerPrice) > parseFloat(bike.price) && (
          <div className="mt-2 text-[11px] text-gray-500">
            MRP <span className="line-through">{formatPrice(bike.customerPrice)}</span>
          </div>
        )}
      </div>
          ...
          <section>
            ...
            {!loading && error && (
              <div className="bg-red-50 border border-red-200 rounded-xl p-6 flex items-start gap-3">
                ...
                <div>
                  <h3 className="text-red-900 font-bold text-xl">Error Loading Bikes</h3>
