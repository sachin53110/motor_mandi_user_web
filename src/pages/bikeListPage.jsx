...
function SkeletonCard() {
  return (
    <div className="bg-white border border-lime-100 rounded-2xl overflow-hidden animate-pulse">
      <div className="h-48 bg-lime-50" />
      <div className="p-4 space-y-3">
        <div className="h-3 bg-lime-100 rounded w-1/3" />
        <div className="h-4 bg-lime-100 rounded w-3/4" />
...
function BikeCard({ bike, onCardClick }) {
  ...
  const savings = bike.price && bike.customerPrice
    ? Math.max(0, parseFloat(bike.customerPrice) - parseFloat(bike.price))
    : 0;

  const discountPercent = bike.price && bike.customerPrice
  ...
  return (
    <div
      onClick={onCardClick}
      className="bg-white border border-lime-100 rounded-2xl overflow-hidden hover:shadow-lg transition-all cursor-pointer transform hover:-translate-y-1"
    >
      {/* Image Container */}
      <div className="relative h-48 bg-gradient-to-br from-lime-50 to-lime-100 overflow-hidden">
        {!imgError && firstImage ? (
          <img
            src={firstImage}
            alt={bike.title}
          ...
          <div className="w-full h-full flex items-center justify-center text-lime-300 text-3xl">
            🏍️
          </div>
        )}
        ...
        <button
          ...
          className="absolute top-3 left-3 w-8 h-8 rounded-full bg-white shadow-md flex items-center justify-center hover:bg-lime-50 transition"
        >
      ...
      <div className="p-4 space-y-2">
        {/* Title */}
        <h3 className="font-bold text-gray-900 text-sm line-clamp-2 hover:text-lime-600 transition">
          {bike.title}
        </h3>
        ...
        <div className="space-y-1 pt-2 border-t border-gray-100">
          <div className="flex items-baseline gap-2">
            <span className="text-lg font-bold text-lime-600">{formatPrice(bike.price)}</span>
            {bike.customerPrice && parseFloat(bike.customerPrice) > parseFloat(bike.price) && (
              <span className="text-xs text-gray-400 line-through">{formatPrice(bike.customerPrice)}</span>
            )}
          </div>
          {savings > 0 && (
            <p className="text-xs text-lime-600 font-semibold">Save ₹{savings.toLocaleString("en-IN")}</p>
          )}
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-black text-gray-900">Premium Bikes</h1>
          ...
          <div className="pb-4 flex gap-3 items-center flex-wrap">
            ...
            <select
              ...
              className="px-4 py-2 border border-gray-300 rounded-full text-sm focus:outline-none focus:border-lime-500"
            >
            ...
            <button
              onClick={() => {
                setCurrentPage(1);
                fetchBikes({ page: 1, limit, condition: condition !== "all" ? condition : undefined });
              }}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="font-bold text-red-900">Error Loading Bikes</h3>
              <p className="text-sm text-red-700 mt-1">{error}</p>
              <button
                onClick={() => fetchBikes({ page: currentPage, limit })}
        ...
        {/* Empty State */}
        {!loading && filteredBikes.length === 0 && !error && (
          <div className="text-center py-16">
            <div className="text-6xl mb-4">🏍️</div>
            <h2 className="text-2xl font-bold text-gray-900 mb-2">No bikes found</h2>
