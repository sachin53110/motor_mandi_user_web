...
export default function PartDetailModal({ isOpen, onClose, item }) {
  ...
  const discount = item.ownerPrice && item.customerPrice
    ? Math.round(
        ((parseFloat(item.customerPrice) - parseFloat(item.ownerPrice)) /
          parseFloat(item.customerPrice)) *
          100
      )
    : 0;

  const savings = item.ownerPrice && item.customerPrice
    ? Math.max(0, parseFloat(item.customerPrice) - parseFloat(item.ownerPrice))
    : 0;

  const typeLabel = item.type === "bike" ? "Bike Part" : item.type === "car" ? "Car Part" : "Part";
    ...
    <>
      ...
      <div className="fixed inset-0 z-[110] overflow-y-auto">
        <div className="min-h-full flex flex-col">
          <div
            className="flex-1 bg-white mt-12 rounded-t-3xl panel-enter flex flex-col"
              ...
              <div className="max-w-6xl mx-auto flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-blue-100 rounded-2xl flex items-center justify-center text-2xl">
                    {item.type === "bike" ? "🏍️" : item.type === "car" ? "🚗" : "🔧"}
                  </div>
                  <div>
                    <h2
                      className="text-blue-950 font-black text-lg leading-none"
                      style={{ fontFamily: "'Bebas Neue', sans-serif" }}
                    >
                      {item.title || typeLabel}
                    </h2>
                    <p className="text-blue-600 text-xs mt-1">{typeLabel}</p>
                  </div>
                </div>
                <button
                  onClick={onClose}
                    ...
                    {item.medias && item.medias.length > 0 && !imgError ? (
                      <>
                        <img
                          ...
                          alt={`${item.title || typeLabel} - View ${imageIndex + 1}`}
                        ...
                        {item.medias.length > 1 && (
                          <>
                            <button
                              onClick={() =>
                                setImageIndex((i) =>
                                  (i - 1 + item.medias.length) % item.medias.length
                                )
                              }
                ...
                <div className="lg:col-span-3 space-y-6">
                  <div className="bg-gradient-to-br from-blue-50 to-teal-50 rounded-2xl p-6 border border-blue-200">
                    <div className="flex items-baseline justify-between mb-3">
                      ...
                      {item.customerPrice && (
                        <span className="text-blue-500/60 text-xs line-through">
                          {formatPrice(item.customerPrice)}
                        </span>
                      )}
                    </div>
                    <div className="text-4xl font-black text-blue-700 mb-2">
                      {formatPrice(item.ownerPrice)}
                    </div>
                    {discount > 0 && (
                      <div className="text-blue-600/70 text-sm">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white border border-blue-100 rounded-xl p-4">
                      ...
                      <div className="text-blue-950 font-bold text-sm line-clamp-2">
                        {item.title || "—"}
                      </div>
                    </div>
                    ...
                    <div className="bg-white border border-blue-100 rounded-xl p-4">
                      ...
                      <div
                        className={`text-sm font-bold px-2 py-1 rounded-full w-fit capitalize ${
                          item.status === "active"
                      ...
                      <div className="space-y-3 pt-4 border-t border-blue-800">
                        ...
                        {item.user.city ? (
                          <div className="flex items-center gap-3 text-blue-300">
                            <MapPin size={18} /> Location: {item.user.city}
                          </div>
                        ) : null}
                      </div>
                    </div>
